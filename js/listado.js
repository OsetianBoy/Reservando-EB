...
Listado.prototype.buscarRestaurante = function(id) {
    var encontrarRestaurant = this.restaurantes.find(function(element){
        return element.id == id;
    });

    return encontrarRestaurant ? encontrarRestaurant : "No se ha encontrado ningún restaurant"; 
}
...
Listado.prototype.obtenerCiudades = function() {
    ...
    return filtrarElementosRepetidos(ciudades).sort();
}

Listado.prototype.obtenerRubros = function() {
    let rubros = this.restaurantes.map(function(restaurant){
        return restaurant.rubros;
    });

    return filtrarElementosRepetidos(rubros).sort();

}

Listado.prototype.obtenerHorarios = function() {
    let arrayHorarios = this.restaurantes.map(function(restaurant){
        return restaurant.horarios;
    });

    var horarios = [];
    arrayHorarios.forEach(function(elem) {
        elem.forEach(function(hor) {
            horarios.push(hor)
        });
    });

    return filtrarElementosRepetidos(horarios).sort();
}
/////////////////////// FILTRO
function filtrarElementosRepetidos(elementos){
    return elementos.filter(function(elem, index, self){
